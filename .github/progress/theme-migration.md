...
- ✅ **2026-02-07**: 清理 _archive 目录，删除所有冗余文件（已完整迁移到 docs/ 或 .ref/）
- ✅ 保留历史文件但不参与构建
...
原 `_posts/` 目录已移至 `_archive/posts/`，但经过清理后发现所有内容都是冗余的：
- 14个 zh-cn 主题文档已完整迁移到 docs/ 目录
- VI Description 子目录是 .ref/VI Description/ 的旧版本副本
- _origin 目录包含不完整的英文草稿和 Jekyll-gitbook 主题示例文件

**2026-02-07 清理**：已删除整个 `_archive/` 目录（~600KB），所有内容已在其他位置保留。
