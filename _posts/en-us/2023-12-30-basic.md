...
A valid CSM module name is a string and should not contain any of the following special characters: `~!@%^&*()\[\]{}+=|\\/?'"<>,.\t\r\n`. These characters are reserved for other purposes within the CSM framework.

### System-Level Module

System-level modules have a "." prefix added to their names, marking them as system-level modules. They functionally behave the same as regular modules, supporting message communication and status subscriptions. The key difference is that system-level modules are not automatically listed in the active module list. This means they won't be directly called in normal logic scheduling and can only be triggered via explicit message sending.

Therefore, system-level modules are typically used to implement global background functionalities, such as the scheduling module in a TCP program.

### CSM Attribute

CSM Attribute is a mechanism within the CSM framework for storing module configurations. It is used for:
- Providing a configuration method that doesn't require message access, allowing external parties to directly read and write module attribute values via module name, attribute name, and data type.
- Providing a mechanism for internal node data sharing in Worker mode and Chain mode. Different nodes share the same Attribute space, and nodes can share data by reading and writing attributes.
