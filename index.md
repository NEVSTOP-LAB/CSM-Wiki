...
- **多框架对比选型** - 相比 DQMH、SMO、Actor Framework，CSM 具有更低的学习曲线，适合不同规模的团队和项目；同一消息可按需选择同步/异步/无返回调用，订阅关系可外部关联无需侵入模块代码，详见 [框架对比 FAQ]({% link docs/faq/framework-selection.md %}#faq-framework-comparison)
