...
LabVIEW Web Service 可以部署在 Windows/Linux 桌面目标上，但**不直接支持** LabVIEW RT Target——RT 目标机通常不运行 NI Application Web Server。在 RT 应用场景下，常见的集成方案是：在 RT 目标上运行 CSM 应用，通过网络变量（Network-Published Shared Variables）或 TCP/IP 将数据上传至 Windows 主机，再由主机端的 Web Service（含 `CSM-RunScript API(POST)`）对外提供 HTTP 接口。
