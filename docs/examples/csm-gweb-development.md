...
graph TB
    subgraph "浏览器（G-Web 前端）"
        GW["G-Web Application<br/>(生成 HTML/CSS/JS)"]
    end

    subgraph "LabVIEW Web Service（Web 后端接口）"
        RS["CSM-RunScript API(POST)<br/>唯一 HTTP 接口"]
    end

    subgraph "LabVIEW 应用（上位机或 RT）"
        Bus[CSM 隐形总线]
        M1[CSM Module A]
        M2[CSM Module B]
        MN[CSM Module N]
...
- **LabVIEW Web Service（Web 后端接口层）**：将 LabVIEW VI 发布为 HTTP 接口。NI Windows 和 RT 应用程序均内置 WebServer，均可发布 Web Service。
- **G-Web 前端（浏览器层）**：G Web Development Software 构建并生成 HTML/CSS/JS 静态文件，由 NI WebServer 托管，用户通过浏览器访问局域网内该项目即可使用。
