...
- Step2: 重新设计了核心字符串解析引擎。
    - Step2.1: JKISM使用[`Parse State Queue.vi`]({% link docs/reference/api-02-core-functions.md %}#parse-state-queuevi)，它只输出下一个状态队列及其对应的参数(位于 >> 之后)，主要用作"出队元素"。
    - Step2.2: CSM 使用[`Parse State Queue++.vi`]({% link docs/reference/api-02-core-functions.md %}#parse-state-queuevi)额外添加了以下主要功能:
