...
使用[`CSM - Convert MassData to Argument With DataType.vim`]({% link docs/reference/api-addon-massdata.md %}#csm-convert-massdata-to-argument-with-datatypevim)生成。
...
使用[`CSM - Convert MassData to Argument.vim`]({% link docs/reference/api-addon-massdata.md %}#csm-convert-massdata-to-argumentvim)生成。
...
使用调试工具监控：
1. 使用[`CSM - MassData Update Status Indicator.vi`]({% link docs/reference/api-addon-massdata.md %}#csm-massdata-update-status-indicatorvi)
...
- Step1: I32数组数据, 此部分选择的打包函数为`CSM - Convert MassData to Argument With DataType.vim`，因此包含DataType部分。
    ...
    - step1.3: 使用[`CSM - MassData Data Type String.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---massdata-data-type-stringvi) 获取MassData 参数字符串中的数据类型。
    - step1.4: 使用[`CSM - Argument Type.vi`]({% link docs/reference/api-03-arguments.md %}#csm---argument-typevi) 获取CSM参数标签，在这里是 <massdata>。
...
- Step2: 波形数据, 此部分选择的打包函数为`CSM - Convert MassData to Argument.vim`，因此不包含DataType部分。
    ...
    - step2.3: 使用[`CSM - Argument Type.vi`]({% link docs/reference/api-03-arguments.md %}#csm---argument-typevi) 获取CSM参数标签，在这里是 <massdata>。
...
MassData 提供了一个帮助函数 [`CSM - MassData Update Status Indicator.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---massdata-update-status-indicatorvi)，用于查看MassData缓存状态。本范例展示了如何使用这个函数。

### Steps

- step1: 使用 [`CSM - Config MassData Parameter Cache Size.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---config-massdata-parameter-cache-sizevi) 设置缓存大小，可以在界面直观的看到这个设置是生效的。
- step2: 每次循环中，构造一组新的数据。
- step3: 使用 CSM MassData 的API对数据进行打包解包，模拟实际程序中的使用情况。
- step4: 使用 [`CSM - MassData Update Status Indicator.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---massdata-update-status-indicatorvi) 更新垂直方向的UI界面缓存状态控件刷新。
- step5: 使用 [`CSM - MassData Update Status Indicator.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---massdata-update-status-indicatorvi) 更新水平方向的UI界面缓存状态控件刷新。
...
- step3: 数据消费者循环
    ...
    - step3.3: (optional) 使用 [`CSM - MassData Update Status Indicator.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---massdata-update-status-indicatorvi) 更新UI界面缓存状态控件刷新。
...
- step2: 数据生产者循环，这个循环不是CSM模块。
    ...
    - step2.2: 使用 [`CSM - Wait and Send Message for Reply.vi`]({% link docs/reference/api-05-module-operation-api.md %}#csm---wait-and-send-message-for-replyvi) 将数据作为 API: Update Waveform 的参数，发送给CSM模块，并标记发送者为Producer。
    - step2.3: 在 Generate 按钮为按下状态时，才发送数据。
    - step2.4: (optional) 使用 [`CSM - MassData Update Status Indicator.vi`]({% link docs/reference/api-addon-massdata.md %}#csm---massdata-update-status-indicatorvi) 更新UI界面缓存状态控件刷新。
...
- step4: 程序退出过程中，使用 [`CSM - Wait and Send Message for Reply.vi`]({% link docs/reference/api-05-module-operation-api.md %}#csm---wait-and-send-message-for-replyvi) 发送同步消息“Macro: Exit” 给 CSM 模块，使CSM模块退出。
## 总结
