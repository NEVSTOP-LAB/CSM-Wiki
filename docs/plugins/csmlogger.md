...
File Logger通过订阅CSM全局日志队列，把运行过程中产生的所有日志写入`.csmlog`文件（纯文本格式，可直接打开查看）。
...
当日志文件超过File Size时，会创建新文件；当文件数超过File Num时，会删除最旧的文件。
...
- **文件格式**：后缀`.csmlog`，可用记事本、VS Code等工具打开
- **时间戳**：格式由`Timestamp format`参数控制
