...
- 必须通过 Worker Agent 名称（去掉 `#` 后缀）发送消息，不能直接与单个 Worker 实例通信。
...
CSM 内置了**责任链模式 (Chain Mode)**，用于按优先级静态路由消息。模块名称末尾使用 `$` 加数字（如 `Handler$1`、`Handler$2`）声明优先级，数字越小优先级越高。

框架会按消息类型从同一 Chain 的注册表中选择“优先级最高且可处理该消息”的节点进行**直接投递**；每条消息只由一个节点处理，不在节点间逐级传递或降级。
...
- 必须通过 Chain 名称发送消息，不能直接向 `ChainName$N` 节点发送消息。
- 若没有任何节点声明可处理该消息类型，该消息将无法被处理（具体行为取决于框架配置）。
