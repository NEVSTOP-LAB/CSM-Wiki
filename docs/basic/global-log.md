...
除了框架自动记录的事件，你也可以手动生成日志：

#### [`CSM - Generate User Global Log.vi`]({% link docs/reference/api-07-global-log.md %}#csm-generate-user-global-logvi)
生成自定义用户日志，适用于记录业务逻辑节点、性能测量点、用户操作等。

**输入**: Log（事件名称）、Arguments（参数）、From Who（来源）、ModuleName（模块名称）、Place（发生地点）
...
将非CSM框架的错误统一记录到全局日志，便于集成第三方库或LabVIEW原生错误。

**输入**: Place（错误位置，默认用调用VI的名称）、Clear Error?（是否清除错误，默认True）

## 工具函数

### 日志转换

- [`Global Log To String.vi`]({% link docs/reference/api-07-global-log.md %}#global-log-to-stringvi): 将日志数据转为字符串。
- [`Global Log To String(Source Time).vi`]({% link docs/reference/api-07-global-log.md %}#global-log-to-stringsource-timevi): 同上，但用发送时间作为时间戳。

### 日志处理

#### [`Global Log History Cacher.vi`]({% link docs/reference/api-07-global-log.md %}#global-log-history-cachervi)
缓存历史日志字符串，支持限制缓存长度、周期性日志折叠和多级处理等级。

**主要输入**: Global Log Data、Length（最大缓存长度，默认10000）、Level（处理等级，更高等级省略信息以提高速度）、Settings（周期性日志配置）

#### [`Auto Processing Level.vi`]({% link docs/reference/api-07-global-log.md %}#auto-processing-levelvi)
根据队列长度变化速度动态推荐处理等级：正常时用Normal等级显示完整信息，日志堆积时提升等级加快处理，稳定后自动降级。

**主要输入**: #Left In Q（队列剩余数量）、Debounce Period（5s）、Period（0.1s）

#### [`Exit With Empty Queue Check.vi`]({% link docs/reference/api-07-global-log.md %}#exit-with-empty-queue-checkvi)
日志监控循环的优雅退出：Stop信号到来时不立即退出，继续处理队列中的剩余日志，队列为空或超时后才真正退出。

**主要输入**: Queue、Stop、Timeout（5s）

## 典型应用场景

- **调试工具开发**: 实时日志窗口、状态仪表板、消息流图等
- **问题诊断**: 错误追踪、时序分析、死锁检测等
- **性能监控**: 消息统计、瓶颈识别、处理时间分析等
- **日志记录**: 操作历史、审计追踪、故障分析等

## 最佳实践

- 开发阶段保留更多日志，生产环境用源端过滤减轻系统负担
- 高频日志使用周期过滤规则
- 优先使用队列方式获取日志，根据堆积情况动态调整处理等级
- 避免在日志处理或高频循环中执行耗时操作
- 在关键业务节点添加自定义日志，使用清晰的名称和参数
- 开发日志工具时注意性能和资源占用，支持搜索、筛选和导出功能

## 参考范例

CSM提供了[多个使用范例]({% link docs/examples/example-csm-advance-example.md %}#全局日志过滤示例)：
...
CSM全局日志系统为调试、监控和问题诊断提供了完整支持，合理使用可大幅提高开发效率。
