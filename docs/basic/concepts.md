...
状态对应Case结构里的一个分支，是CSM的基本逻辑单元。与其他状态机不同，CSM用字符串驱动状态跳转，并定义了特定的语法规则。

宏状态是多个状态的组合，相当于一组固定的状态序列。CSM模板默认有两个宏状态：`Macro: Initialize`（启动时）和`Macro: Exit`（退出时）。

## 消息队列(Message Queue)

消息队列是CSM运行的核心机制。每个CSM模块维护两个队列：
...
消息是CSM模块间1对1通讯的方式。消息本质上是符合特定语法的字符串，包含状态名、参数、消息类型等信息。例如模块A发送"API: DoSth"给模块B，B就会进入对应状态。CSM提供了辅助VI来生成消息字符串，无需手写。
...
- 目标为空字符串时产生 `NO Target Error`，目标不存在时产生 `Target Error`，超时时产生 `Target Timeout Error`
...
响应是被调用模块的返回字符串。例如A调用B的"API: DoSth"，B处理完后返回"Received"表示收到。模板中响应默认为空字符串。
...
| `Response Info` 簇中的 `Argument` | **原始请求的参数**（发送消息时携带的参数） |

这一设计的目的是在处理响应时具备完整的上下文信息。例如发送 `Query ID >> 李梅 -@ database` 后，进入 `Response` 状态时：
- CASE `Argument` = `12345678`（被调用方返回的证件号码）
- `Response Info.Argument` = `李梅`（原始请求中的查询参数）
...
在同步/异步消息的 `Response` / `Async Response` 状态中，可以从 `Additional Information` 获取被调用方的错误信息，也可以通过 `Source CSM` 输出得知返回值来自哪个模块。`Response Info` 簇中的 `Argument` 字段携带的是**原始请求参数**，而非调用返回值；调用返回值位于 CASE 的 `Argument` 输出中。
...
详细用法看[参考文档]({% link docs/reference.md %})。

### 参数数据类型

由于参数只能是字符串，传递复杂数据时需要先编码、收到后再解码。CSM内置了三种编码方案：

- **HEXSTR**：将任意LabVIEW数据序列化为十六进制字符串，格式为 `<HEXSTR>十六进制字符串`。适合传递簇、数组等复杂类型，但数据量大时字符串较长。
...
含UI的CSM模块通常用**事件结构**（Event Structure）响应用户操作。事件结构会阻塞等待事件，但外部消息到来时需要立即跳出处理——CSM通过以下机制解决：
...
- **模块、状态、消息、广播**是基础构成
- **消息队列**驱动状态机运行，分高优先级（同步/中断）和低优先级（异步/信号）两级
- **同步/异步**两种通讯方式，同步支持全局超时控制
- **预置状态**（Response、Async Response、Error Handler 等）处理通讯生命周期
- **订阅机制**实现1对多通知
- **参数和属性**承载数据，参数通过 HEXSTR/ERRSTR/SAFESTR 编码支持复杂类型
- **事件结构支持**让含UI的模块能在等待用户操作的同时响应外部消息

掌握这些概念后即可开始使用CSM，具体用法请参阅其他章节。
