...
- 会在Response状态收到返回
- 可能出现超时、目标不存在等错误
- 默认超时时间为 `-2`（使用全局设置），可通过 [`CSM - Set TMO of Sync-Reply.vi`]({% link docs/reference/api-04-management-api.md %}#csm---set-tmo-of-sync-replyvi) 统一调整；异步消息不受超时限制
...
CSM框架预置了若干标准状态，用于处理通讯生命周期中的各类场景。每个CSM模块都应包含这些状态的处理逻辑：

| 预置状态 | 触发时机 |
|---|---|
| `Response` | 收到同步消息的返回值时自动进入 |
| `Async Response` | 收到异步消息的返回值时自动进入 |
| `Async Message Posted` | 发出异步消息后立即进入，用于跟踪已发送的异步请求 |
| `Error Handler` | 模块发生错误时自动进入，并广播 `Error Occurred` 状态通知外部 |
| `Critical Error` | 框架级严重错误（如模块名重复）时进入，模块将停止运行 |
