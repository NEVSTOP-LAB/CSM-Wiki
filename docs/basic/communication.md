...
| 异步调用（无返回） | `->|` | 否 | 单向通知 |
| 信号广播 | `<status>` / `<broadcast>` / `<all>` | 否 | 向所有订阅者广播状态 |
...
{: .note }
> 在 `Response` 状态中，**`Parse State Queue++.vi` 的 `Argument`** 是被调用方的返回值，**`Parse State Queue++.vi` 的 `Response Info.Argument`** 是发送请求时携带的原始参数。两者同时可用，让响应处理逻辑具备完整上下文。
...
{: .note }
> 在 `Async Response` 状态中，**`Parse State Queue++.vi` 的 `Argument`** 是被调用方的返回值，**`Parse State Queue++.vi` 的 `Response Info.Argument`** 是发送请求时携带的原始参数。对于异步调用尤为重要——由于返回时间不确定，多个异步请求可能同时在途，携带原始参数让响应处理逻辑更加清晰。
...
| -------- | ---- | ------ | -------- | -------- |
| 信号广播 (Status) | `<status>` / `<broadcast>` / `<all>` | 低（与异步消息相同） | 显式调用 | 主动发送事件通知（如数据采集完成） |
...
ModuleStatus >> Arguments -> <all>
```

{: .warning .callout-hover }
> **避免混用类型符号**
>
> `<status>-><broadcast>` 同时出现两个广播类型符号属于不推荐的写法，可能导致未知行为。请只保留一个符号（如 `<status>`）。
...
{: .warning .callout-hover }
> **常见误解：同步等待期间可同时发出异步消息**
>
> 在同一个状态队列中，同步消息（`-@`）会阻塞队列的继续执行，后续消息（包括异步消息）**必须等到同步调用返回后才会被处理**。并不存在"同步等待期间同时发出异步消息"的效果。

```csm
API: GetData >> params -@ Database        // 同步调用，阻塞等待返回
API: UpdateProgress >> 50% ->| UI         // 必须等上一条同步消息返回后才会执行
```

如果需要在同步调用进行的同时发出异步消息，需要从**不同的模块或线程**发送，例如：

- **方案 1：从另一个独立运行的 CSM 模块发送**——两个模块并行运行，彼此不互相阻塞
- **方案 2：从外部非阻塞线程发送**——在独立线程中通过 [`CSM - Post Message.vi`]({% link docs/reference/api-05-module-operation-api.md %}#csm---post-messagevi) 发出，不受其他同步调用影响
